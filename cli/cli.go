...
func (c App) String() string {
	return fmt.Sprintf("[verbose=%t, local=%s, remote=%s, server=%s, key=%s, verbose=%t, help=%t, version=%t, detach=%t, insecure=%t, keep-alive-interval=%s, connection-retries=%d, retry-wait=%s]",
		c.Verbose, c.Local, c.Remote, c.Server, c.Key, c.Verbose, c.Help, c.Version, c.Detach, c.Insecure, c.KeepAliveInterval, c.ConnectionRetries, c.WaitAndRetry)
}
